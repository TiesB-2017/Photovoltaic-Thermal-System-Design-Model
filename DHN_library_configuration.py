...
def update_ATES(active, T_in, T_0, T_soil, Qdot_SC=0, Qdot_HP=0, Tmax=95 + 273, Tmin=5 + 273, mdot=50, T_network=15 + 273, efficiency=0.8, L_ATE=10, r=250, c_w=4200, c_r=830, n=0.2, dt=0.25*3600, rho_f=1050, rho_r=1602):  # , Qdot_SD = 100
    import math
...
def Qdot_ATES2Soil(T_ATES, depth=150, L_ATE=10, k_soil=1.19, r=250, T_soil=11):
...
#    Qdot_TESS_SD = []
    ...
    for step in range(t_final):
        ...
        if Thermal_Components[2]:
            ...
            [Q_dot_SC, T_PVT_layers, T_tank_PVT, T3, Qdot_PVT2Network, Qdot_PVT2TESS] = Qdot_PVT(PVT_active, PVT_TESS_charge, int(
                T_amb[step] - 273), G[step]/3600, 1, T2-273, T_PVT_layers[0], T_PVT_layers[1], T_PVT_layers[2], T_PVT_layers[3], T_tank_PVT, T_ATES[-1]-273, m_f_dot=0.029085, n_HE=1, t_end=int(15*60), n_modules=n_modules)
            T3 += 273
            ...
            fluid_registry[4].append(T3 - 273)
        ...
        if Thermal_Components[1]:
            ...
            ATES_state = update_ATES(TESS_active, T4, T_ATES[-1], T_soil=Tsoil)
            P_pump = P_ATES_pump()
        ...
        Q_ATES_total.append(Qdot_ATES_losses)
        T_ATES_in = 0
        Total_CO2_grid = 0
        Total_CO2_PV = 0
        Total_CO2 = 0
