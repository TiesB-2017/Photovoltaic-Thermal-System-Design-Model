...
def Update_PVT_tank(T_in_PVT, T_in_Network, T0, T0_ATES, PVT_active=True, TESS_charge=False, mdot=10, m=2000, c=4200, efficiency_TESS=0.8, dt=1):
...
def update_ATES(active, T_in, T_0, T_soil, Qdot_SC=0, Qdot_HP=0, Tmax=95 + 273, Tmin=5 + 273, mdot=50, T_network=15 + 273, efficiency=0.8, L_ATE=10, r=144, c_w=4200, c_r=830, n=0.2, dt=0.25*3600, rho_f=1050, rho_r=1602):  # , Qdot_SD = 100
    import math
    Qdot_soil = Qdot_ATES2Soil(T_0 - 273)
    ...
    return [T_new, Qdot_soil]


def Qdot_ATES2Soil(T_ATES, depth=150, L_ATE=10, k_soil=1.19, r=144, T_soil=11):
...
def Thermal_Electrical_model(Thermal_Components, Q_storage_switch, T_in_0, T5, T6, T7, T_ATES_0, T_tank_PVT, T_PVT_layers, TESS_min_op_T, TESS_max_op_T, SoC_0_BESS, T_amb, T_set, G, Tsoil, P_Load_new, t_final, T_s_0, n_modules, radius_ATES, PowerPrices_15min_list, start_time, Intensity_array, P_BESS_max=1.28, P_Grid_max=0, Capacity_BESS=3.36, PVT_2_TESS=False, HP_2_TESS=False, Network_losses=False, m=4000, T_sup=50 + 273, T_TESS_max=273 + 95, m_dot=10, m_dot_pipe=50, c_f=4200, A_PVT=1.77, dt=60*15, L_1 =100, epsilon = 0.9, alpha_0 = 0.1, k_soil = 1.19, depth = 6, v = 1, HE_eff = 0.8):
    ...
    street = 17
    T_in =  [[T_in_0, T_in_0] for _ in range(street)]
    T_ATES = [T_ATES_0]
    T_s = [T_s_0]

    fluid_registry = [[], [], [], [], [T5 - 273]]
    T_registry = [[[] for _ in range(street)], [[] for _ in range(street)], [[]], [[]], [[]], [[]], [[] for _ in range(street)], [[]]]
    T_tank_PVT_registry = []
    Qdot_PVT_registry = []
    Qdot_SC_TESS = []
    Qdot_PVT_Network = []
    Qdot_pipe1 = []
    P_PVT = []
    T_ATES_out = [[] for _ in range(street)]
    T_ATES_in = [[] for _ in range(street)]
    A_PVT = A_PVT*n_modules
    
    
    P_HP = [[] for _ in range(street)]
    Qdot_HP = [[] for _ in range(street)]
    Qdot_evap = [[] for _ in range(street)]  
    COP = [[] for _ in range(street)]
    COP_new = [[3.1] for _ in range(street)]
    Qdot_Losses = [[] for _ in range(street)]
    Qdot_network = [[] for _ in range(street)] 
    Qdot_pipes = [[] for _ in range(street)]
    P_HP_total = []
    Q_HP_total = []
    Q_evap_total = []
    Q_Losses_total = []
    ...
    Q_ATES_total = []
    P_ATEP_total = []
    masscapacity_T = [[] for _ in range(street)]
    Netheat1 = []
    Surplus_power = []
    Q_storage = []
    Profit = []
    ...
    Qdot_HP_TESS = []
    Qdot_ATES_losses = [[] for _ in range(street)]


    SoC_BESS = [SoC_0_BESS]
    P_BESS = []
    P_ATE_pump = [[] for _ in range(street)]

    P_Grid = []
    Netpower = []
    
    Pipelengths = [30, 45, 45, 50, 80, 110, 30, 30, 80, 80, 60, 50, 40, 30, 50, 30, 70]        
    Housetypes = [1, 1, 1, 2, 1, 4, 1, 1, 1, 3, 1, 4, 1, 2, 1, 2, 2]
    ...
    for step in range(t_final):
        ...
        for i in range(street):
            ...
            if Housetypes[i] == 4:
                if Thermal_Components[1]:
                    ...
                    T_ATES_in[i].append(T7)
                    
                    ATES_state = update_ATES(TESS_active, T7, T_ATES[-1], T_soil=Tsoil)
                    P_pump = P_ATES_pump()
                    P_ATE_pump[i].append(P_pump)
                    T_ATES.append(ATES_state[0])
                    Qdot_ATES_losses[i].append(ATES_state[1])
                    T1 = T_ATES[-1]
                else:
                    T_ATES.append(0)
                    T1 = T7
                ...
                masscapacity_T[i].append(0)
                Qdot_Losses[i].append(0)
                Qdot_network[i].append(0)                
                T_in[i].append(0)
                
                T_ATES_out[i].append(T2)
        ...
        for i in range(street):
            ...
            if len(Qdot_ATES_losses[i]) > 0:
                Qdot_ATES_total += Qdot_ATES_losses[i][-1]
        ...
        Q_ATES_total.append(Qdot_ATES_total)
        ...
        if Thermal_Components[1]:
            ...
            ATES_state = update_ATES(TESS_active, T2, T_ATES[-1], T_soil=Tsoil)
            P_pump = P_ATES_pump()
            P_ATE_pump[0].append(P_pump)
            
            T_ATES.append(ATES_state[0])
            Qdot_ATES_losses[0].append(ATES_state[1])
        ...
        for i in range(street):
            if len(P_ATE_pump[i]) > 0:
                Pump_total += P_ATE_pump[i][-1]
        
        P_ATEP_total.append(Pump_total)
        ...
        Netpower.append(P_PVT[-1] - P_HP_total[-1] - P_ATEP_total[-1] - P_Load_new[step]*1000)
        ...
        if Q_storage_switch == True:
            ...
            if  Netpower[-1] > 0:
                ...
                Q_dot_extra = HP_extra(int(T_amb[step]) - 273, T5 - 273, Netpower[-1])    
                Q_storage.append(Q_dot_extra)
            
                T7 = T5 + HE_eff*Q_dot_extra/(m_dot_pipe*c_f)
                Netpower_storage.append(P_PVT[-1] - P_HP_total[-1] - P_ATEP_total[-1] - P_Load_new[step]*1000 - Netpower[-1]) 
                
                
            else:
                T7 = T5
                Q_storage.append(0)
                Netpower_storage.append(P_PVT[-1] - P_HP_total[-1] - P_ATEP_total[-1] - P_Load_new[step]*1000) 
